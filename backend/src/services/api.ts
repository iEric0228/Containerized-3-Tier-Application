...
router.get('/logs', async (req, res) => {
  try {
    ...
    const limit = Number.isFinite(rawLimit) && rawLimit >= 1 && rawLimit <= 1000 ? rawLimit : 100;

    const response = await axios.get(`${lokiUrl}/loki/api/v1/query_range`, {
      params: {
        query,
        limit,
        start: Date.now() - (5 * 60 * 1000) * 1000000,
    ...
    logger.error('Failed to fetch Loki logs:', error);
